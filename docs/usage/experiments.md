...
- `amazonbook-edgrec-candidate-search` is the AmazonBook-specific Optuna comparison for the `compact` and `deep_features` EDGRec profile families under one `ValidationCRRU@20_40` objective/revision. Use it before promoting either AmazonBook EDGRec candidate into a thesis-facing formal profile.
...
- The CRRU objective `ValidationCRRU@20_40` uses the same absolute component/exponent structure as report CRRU, including raw PyG AveragePopularity normalized inside CRRU by the logged or reconstructed largest train item count, plus inverse-log peak-VRAM/seconds-per-epoch resource penalties. Search runs skip final test evaluation; test metrics remain reserved for promoted confirmation profiles.
