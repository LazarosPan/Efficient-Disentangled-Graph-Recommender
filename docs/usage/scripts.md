...
- `query-results`: inspect the thesis SQLite database. The base command renders the default thesis summary for full-data formal runs and currently supported public ablation variants, ordered by dataset and then by `CRRU@20` / `CRRU@40`. Each section is split into accuracy, bias/diversity, and composite-resource tables so thesis-facing tables stay readable. `CRRU@20` and `CRRU@40` are framed as Composite Resource-aware Recommendation Utility at K with dataset-local section-row min-max normalization and are not causal-effect estimators. Per-run details still include `Resources:`, diagnostics, and full canonical experiment names, then the command writes the report to `results/query_results.md`. See [../../.agents/skills/edgrec-implementation/edgrec-training.md](../../.agents/skills/edgrec-implementation/edgrec-training.md) for the LaTeX definition.
