...
- `quick-validate` is a zero-argument smoke command with one fixed tiny runtime shape and no SQLite/MLflow persistence, so validation follows the active recipe/ablation semantics without writing smoke rows.
...
- `query-results`: inspect the thesis SQLite database. The base command renders the default thesis summary for full-data formal runs and currently supported public ablation variants, ordered by dataset and then by `CRRU@20` / `CRRU@40`. CRRU is reported as an absolute per-run Composite Resource-aware Recommendation Utility at K, with raw PyG AveragePopularity normalized inside CRRU by the logged or reconstructed largest train item count, peak GPU memory as a capacity cost, and epoch duration as a throughput cost. It is not a causal-effect estimator or standard recommender metric. Per-run details still include `Resources:`, diagnostics, and full canonical experiment names, then the command writes the report to `results/query_results.md`. See [../../.agents/skills/edgrec-implementation/edgrec-training.md](../../.agents/skills/edgrec-implementation/edgrec-training.md) for the LaTeX definition.
...
- `export-optuna-figures`: write Optuna PNG diagnostics under `results/optuna_figures/`. The default thesis set includes dataset-local optimization history, importance comparison, CRRU component diagnostics, parameter landscapes, branch-depth heatmaps, and fanout/runtime tradeoffs; importance rows with enough same-revision fresh completed trials are marked `strict`, while older all-completed univariate association fallbacks are marked `exploratory`.
