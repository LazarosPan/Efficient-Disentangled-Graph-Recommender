...
Auto-batch CUDA OOM decisions are logged with the original exception summary, PyTorch allocator allocated/reserved/peak memory, and probe-stage/subgraph dimensions when available so resource-monitor readings can be compared with actual allocator pressure. The experiment runner sets `PYTORCH_ALLOC_CONF=expandable_segments:True` before importing `torch` unless the user already supplied a CUDA allocator policy, and it accepts legacy `PYTORCH_CUDA_ALLOC_CONF` as a source value.
