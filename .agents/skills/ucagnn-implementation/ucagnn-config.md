...
- `.agents/skills/ucagnn-implementation/ucagnn-config.md`
- `src/utils/config.py`
...
| `lightgcn_paper` preset (`UCaGNNConfig.preset_lightgcn_paper()`) | Paper-faithful LightGCN adapter: `PaperLightGCN`, single branch, full-graph propagation during training, observed graph only, no dropout, Adam optimizer with constant `lr=0.001` (`lr_scheduler="none"`), explicit ego-embedding L2 with `weight_decay=1e-4`, `batch_size=2048`, and no sampled-neighbor fan-out in the optimizer step. |
...
| Item branch capacity | `separate_item_branch_embeddings` | Default `False` keeps one shared item table; `True` gives dual-branch U-CaGNN explicit interest/conformity item tables while retaining `"item"` fallback compatibility. |
...
- `separate_item_branch_embeddings=False` is the default; enable it only through U-CaGNN search/profile overrides when testing DICE-like item branch capacity.
...
| `ucagnn-mechanism-coarse` | `core-ucagnn-mainline` | KuaiRec_v2, MovieLens1M, KuaiRand1K; no AmazonBook broad pass | `ValidationAccuracy@20_40` | TPE seed 42, startup 40; Hyperband min 6, factor 3, bootstrap 1 | `max_epochs=60`, `trials=120`, `parameters` declare profile labels and sibling `profile_overrides` maps them to concrete score-fusion, item-branch, context/feature, loss, and graph-depth config overrides before `UCaGNNConfig` is built; dataset-local scalar overrides cover MovieLens `lr=0.005`/score floor `0.1` and wider KuaiRand DICE margins |
| `ucagnn-cagra-accuracy-ablation` | `core-ucagnn-mainline` | KuaiRec_v2, MovieLens1M, KuaiRand1K | `ValidationAccuracy@20_40` | TPE seed 42, startup 8; Hyperband min 6, factor 3, bootstrap 1 | `max_epochs=60`, `trials=24`, compares `graph_policy=observed` and `cagra_augmented` plus optional `cagra_candidate_k` |
...
- CAGRA is not part of the default core or mechanism Optuna spaces because `graph_policy="cagra_augmented"` changes the training graph and `cagra_candidate_k > 0` changes evaluation candidate coverage. Use `ucagnn-cagra-accuracy-ablation` or `ucagnn-cagra-ablation` for that systems/retrieval ablation, and keep `cagra_candidate_k=0` as the full-catalog baseline for formal ranking tables.
