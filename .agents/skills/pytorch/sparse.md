torch.sparse (https://docs.pytorch.org/docs/2.12/sparse.html)

Created On: Apr 26, 2017 | Last Updated On: Apr 13, 2026
...
Mdense=r×c×eMsparse=Mspecified+Mmetadata=r×c2×e+r×c2×2=rce2+rc=rce(12+1e)
Mdense​=r×c×eMsparse​=Mspecified​+Mmetadata​=r×2c​×e+r×2c​×2=2rce​+rc=rce(21​+e1​)
...
C=MsparseMdense=12+1e
C=Mdense​Msparse​​=21​+e1​
...
torch.sparse.semi_structured.to_sparse_semi_structured(original_tensor, transposed=False, alg_id=0)
[source]

    This function converts a dense tensor into a sparse semi-structured tensor. It will return a SparseSemiStructuredTensor, a subclass of torch.Tensor.
    ...
    Parameters:
            ...
            alg_id (int, optional) – the algorithm id to use for cuSPARSELt matmul. Defaults to 0. Can be obtained via torch._cslt_sparse_mm_search.

    Returns:

        A sparse semi-structured tensor created from the given original_tensor
    Return type:
...
Calling torch.Tensor._values() will return a detached tensor. To track gradients, torch.Tensor.coalesce().values() must be used instead.
...
Constructs a sparse tensor in Compressed Sparse format - CSR, CSC, BSR, or BSC - with specified values at the given compressed_indices and plain_indices.
...
This function does exact same thing as torch.addmm() in the forward, except that it supports backward for sparse COO and CSR matrix mat1.

sparse.sampled_addmm
	

Performs a matrix multiplication of the dense matrices mat1 and mat2 at the locations specified by the sparsity pattern of input.
...
Performs a matrix multiplication of a sparse COO matrix mat1 and a strided matrix mat2.
