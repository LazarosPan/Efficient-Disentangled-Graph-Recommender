...
| `amazonbook-edgrec-deep-features-candidate` | AmazonBook-specific richer EDGRec candidate. | Explicit deeper/use_features/contrastive candidate preserved from prior Optuna evidence; feature gate fixed by profile; compare against compact EDGRec and `lightgcn_paper` accuracy before full-data thesis use. |
...
- The default compact formal candidate remains `edgrec-compact-search-prior` for compatibility with existing search spaces. Use `edgrec-global-top-<dataset>-r<rank>` profiles for current dataset-local validation-selected test-profile runs.
- Validation-selected EDGRec formal profiles omit `single_branch_gnn_layers`; dual-branch depth is owned by `interest_gnn_layers` and `conformity_gnn_layers`.
...
- Evidence note: Optuna supports compact defaults for KuaiRec_v2, MovieLens1M, and KuaiRand search/probing. AmazonBook is not excluded from EDGRec, but current evidence does not prove its best hyperparameters; run dataset-specific compact-vs-deep formal/search comparisons before full-data thesis use.
