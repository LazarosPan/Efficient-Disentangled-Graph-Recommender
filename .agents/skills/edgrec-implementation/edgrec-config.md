...
| `edgrec-global-top-<dataset>-r<rank>` | Dataset-local promoted EDGRec candidates. | Top 5 distinct completed Optuna configs per dataset by global validation `ValidationOnlineCRRU@20_40`; formal budget `epochs=300`, `patience=20`, observed graph only, `auto_batch_size=false`, and fixed batch sweep `[trial batch, 2x trial batch]`; added to final formal-row reporting. |
