...
| `amazonbook-edgrec-candidate-search` | `amazonbook-edgrec-compact-candidate` | AmazonBook only | `ValidationOnlineCRRU@20_40` | TPE seed 42, startup 4; Hyperband min 15, factor 3, bootstrap 1 | `max_epochs=150`, `trials=12`, compares `compact` and `deep_features` profile-family labels under one objective/revision; current evidence does not prove best AmazonBook hyperparameters |
...
- The default compact formal candidate is `edgrec-compact-search-prior` and targets `movielens1m`, `kuairec_v2`, and `kuairand1k`. AmazonBook is excluded only from this shared compact default queue: current evidence is inconclusive and dataset-specific, so AmazonBook requires compact-vs-deep EDGRec comparison plus `lightgcn_paper` accuracy framing before thesis promotion. Development, preprocessing-sweep, runtime-probe, `taobao`, `movielens20m`, and broader core-comparison profiles remain explicit instead of default catalog entries.
