...
| `uv run scripts/quick_validate.py` | Fixed smoke suite over the shared runtime path. |
...
3. `build_runtime_model()` derives item recency from `data.train_mask` and instantiates `UCaGNN`.
...
- evaluation-only changes such as `eval_scoring_mode` may reuse the same checkpoint,
