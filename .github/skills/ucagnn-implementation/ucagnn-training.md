...
- The main training path now avoids synchronized per-stage profiling. The default experiment logs keep epoch wall time, total training time, and peak VRAM, while sign-aware scalars (`alpha_pos`, `alpha_neg`) continue to flow through the metric logger when they exist.
...
- The public orchestration surface is now intentionally thin: `formal-run` exposes only `--profile`, `--list-profiles`, and `--overwrite-checkpoint`, while `ablation` exposes only dataset selection, variant selection, and checkpoint overwrite. Internal benchmark batching still uses batch ids/state files, but that execution plumbing stays behind the public thesis workflow.
...
- The formal-run payload now normalizes its config-bearing fields through one shared benchmark-config contract in `experiments/run_experiment.py::normalize_benchmark_config_overrides()`, and each concrete matrix item rebuilds a run-local config input dict through `experiments/run_experiment.py::build_benchmark_config_inputs()` before calling `build_config()`. Quick validation now reuses the same runtime config-input builder (`build_runtime_config_inputs(...)`) instead of hand-assembling a parallel mapping, and both builders now share the same internal present-field config-input helper so the same `num_neighbors` field can represent either one concrete fan-out vector or a formal sweep of several vectors without splitting the contract across parallel option names. That shared benchmark payload now also carries follow-up experiment knobs such as `use_features`, `feature_policy`, `preprocessing_preset`, and `derived_split_mode`, while train popularity is always derived from the observed training interactions rather than a separate popularity-window override.
...
- The current default formal profile is development-focused and keeps the day-to-day formal matrix narrow around the proposed architecture. Exact support-parameter choices belong in `experiments/experiment_catalog.json`, not in this routed implementation summary.
...
- `experiment_summary` now keeps both `avg_epoch_time_s` (mean per-epoch wall clock) and `training_time_s` (total wall clock for the finished run), plus `completed_train_epochs` derived from the logged training epochs. The top completed-runs table in `scripts/query_results.py` should therefore display **Training Time** from `training_time_s` and the actual stopped epoch count from `completed_train_epochs`, not the configured epoch budget from `config_json`.
...
## MLflow Routing
- `experiments/run_experiment.py` resolves MLflow tracking from the active run args first, then `MLFLOW_TRACKING_URI`, then the project default `results/mlflow.db`.
...
- `quick_validate.py` is now the unified tiny-scale pipeline validator. By default it exercises the canonical recipe matrix, all ablation variants, observability probes across the selected datasets (checkpoint/resume and feature path), and evaluation scoring modes with aggressive row caps and sampled interactions.
...
- `quick-validate` is a zero-argument smoke command; the default command is intended to be the single broad post-change validation entry point.
...
- `query-results` now supports only `--view` on its public CLI, with the logger-owned `completed`, `attention`, `errors`, and `comparison` views covering the supported inspection paths.
- The default `query-results` summary is thesis-facing: it keeps only full-data formal and ablation rows, orders them by dataset then test ranking metrics, includes the full test suite (`NDCG`, `Recall`, `HitRatio`, `Personalization`, `AveragePopularity` at 20 and 40), restores resource readouts (`training_time_s`, `completed_train_epochs`, `peak_vram_mb`, `avg_gpu_utilization_pct`) on a dedicated `Resources:` line, prints the full canonical experiment name on its own `Experiment:` line instead of truncating it inside the metric table, and writes the rendered default report to `results/query_results.md`.
