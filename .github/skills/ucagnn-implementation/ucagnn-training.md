...
- Keep the saved `benchmark_args` payload normalized once at the boundary: the current-format dict should drive JSON persistence, semantic plan matching, and resumed execution itself, and stale saved fields should require a fresh formal run instead of reviving compatibility restore code.
