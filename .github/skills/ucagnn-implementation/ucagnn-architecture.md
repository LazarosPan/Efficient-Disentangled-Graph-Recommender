...
- Cached evaluation keeps the narrow public model surface: `UCaGNN.score_users_from_propagated()` owns the evaluator-facing full-score path, `UCaGNN.get_all_score_components()` remains the diagnostic full-catalog entry point, and score payloads expose `counterfactual_score` as the single public counterfactual key.
