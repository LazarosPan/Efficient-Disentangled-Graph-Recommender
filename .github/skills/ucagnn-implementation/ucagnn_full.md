...
The formal experiment matrix is defined along two semantic axes. Profile-owned `scoring_weight_modes` can add explicit score-mix comparisons when needed:
...
| `preset_dice_like()` | DICE-inspired dual-branch | Zheng et al. 2020 GCN-DICE | Train-interaction bipartite graph with optional ANN augmentation | `L_rec + L_interest_bpr + L_conformity_bpr + L_independence` |
| `preset_full()` | Full U-CaGNN | U-CaGNN thesis contribution | Train-interaction bipartite graph with optional ANN augmentation | `L_rec + L_interest_bpr + L_conformity_bpr + L_independence + L_contrastive + L_pop` |
...
- `use_early_stopping=True`: keeps non-formal runs aligned with the current best-checkpoint workflow. Formal profiles may override this when a protocol needs a fixed training horizon.
