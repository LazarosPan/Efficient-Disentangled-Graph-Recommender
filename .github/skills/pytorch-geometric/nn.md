...
```python
class Sequential(input_args: str, modules: List[Union[Tuple[Callable, str], Callable]])
```
...
```python
class Linear(in_channels: int, out_channels: int, bias: bool = True, weight_initializer: Optional[str] = None, bias_initializer: Optional[str] = None)
```
...
```python
class HeteroLinear(in_channels: int, out_channels: int, num_types: int, is_sorted: bool = False, **kwargs)
```
...
```python
class HeteroDictLinear(in_channels: Union[int, Dict[Any, int]], out_channels: int, types: Optional[Any] = None, **kwargs)
```
...
forward(x_dict: Dict[str, Tensor])→ Dict[str, Tensor]
Forward pass.
...
An aggregation operator that returns the feature-wise 
-th quantile of a set 
.
...
Performs Deep Sets aggregation in which the elements to aggregate are first transformed by a Multi-Layer Perceptron (MLP) 
, summed, and then transformed by another MLP 
, as suggested in the "Graph Neural Networks with Adaptive Readouts" paper.
...
The Learnable Commutative Monoid aggregation from the "Learnable Commutative Monoids for Graph Neural Networks" paper, in which the elements are aggregated using a binary tree reduction with 
 depth.
...
A base class to perform fast 
-nearest neighbor search (
-NN) via the faiss library.
...
-nearest neighbor search (
-NN) based on the 
 metric via the faiss library.

MIPSKNNIndex

Performs fast 
-nearest neighbor search (
-NN) based on the maximum inner product via the faiss library.

ApproxL2KNNIndex

Performs fast approximate 
-nearest neighbor search (
-NN) based on the the 
 metric via the faiss library.
...
The Deep Graph Infomax model from the "Deep Graph Infomax" paper based on user-defined encoder and summary model 
 and 
 respectively, and a corruption function 
.
...
The correct and smooth (C&S) post-processing model from the "Combining Label Propagation And Simple Models Out-performs Graph Neural Networks" paper, where soft predictions 
 (obtained from a simple base predictor) are first corrected based on ground-truth training label information 
 and residual propagation.
...
Here, each MessagePassing instance 
 ...
 (one instance for each relation in 
), and message passing in layer 
 is performed via
...
where 
 ...
 under relation 
, and 
 denotes the aggregation scheme aggr to use for grouping node embeddings generated by different relations ("sum", "mean", "min", "max" or "mul").
...
Here, each MessagePassing instance 
 ...
 (one instance for each basis in num_bases), and message passing in layer 
 is performed via
...
where 
 denotes the neighborhood of 
 under relation 
. Notably, only the trainable basis coefficients 
