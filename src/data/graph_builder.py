...
neighbours) receive a neutral sign of 0.0.
"""
...
def build_graph(
    ...
    Args:
        ...
        embeddings: (n_users + n_items, D) node embeddings for knn/cagra methods.
                    Required when graph_method != "dense".
    ...
    # Popularity as node feature (items only)
    popularity = torch.from_numpy(canonical.popularity).float()
...
def _build_knn(
    ...
    Falls back to dense if no embeddings provided (first epoch).
