...
class CachedPropagationTrainer:
    ...
    def train(
        ...
        for epoch in range(start_epoch, self.config.epochs):
            ...
            for batch_idx, start in enumerate(range(0, n_train, self.config.batch_size)):
                ...
                with profile_stage("forward", self.profiler):
                    pos_scores = self.model.scoring(propagated, batch_users, batch_pos_items)
                    neg_scores = self.model.scoring(propagated, batch_users, batch_neg_items)

                    output = self._build_output(
                        propagated, init_embs, pos_scores, neg_scores,
    ...
    def _build_output(
        ...
        if self.config.use_ipw:
            ...
            result["ipw_weights"] = torch.ones(
                batch_users.size(0), device=batch_users.device,
            )
